...
local function list_dir(path)
    ...
    if is_windows() then
        ...
        local function read_files_cmd(cmd)
            local fh = io.popen(cmd)
            if not fh then return end
            for name in fh:lines() do
                if not should_ignore(name) then
        ...
        read_cmd(     'dir /b /ad  "' .. path .. '" 2>nul')
        read_files_cmd('dir /b /a-d "' .. path .. '" 2>nul')
    else
        local fh = io.popen('ls -1A "' .. path .. '" 2>/dev/null')
        if not fh then return dirs, files end
        local all = {}
        for name in fh:lines() do
            if not should_ignore(name) then all[#all + 1] = name end
        end
        fh:close()
        for _, name in ipairs(all) do
            if is_dir(path .. "/" .. name) then
                dirs[#dirs + 1] = name
            else
                files[#files + 1] = name
            end
...
local function render(root_name, lines, output_md)
    ...
    if output_md then
        print("```")
        print("")
        print("> *Generated by [filetree](https://github.com/KARAN-D05) — Lua tree generator*")
    end
    print("")
end
